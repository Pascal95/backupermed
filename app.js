...
app.post('/api/users/login', async (req, res) => {
    try {
      ...
      console.log("Email:", email);  // Log l'email reçu
  
      const user = await User.findOne({ where: { email } });
      console.log("User trouvé:", user);  // Log l'utilisateur trouvé ou non
      ...
      console.log("Mot de passe valide:", isPasswordValid);  // Log la validation du mot de passe
      ...
      console.log("FicheUser trouvé:", fiche);  // Log la fiche utilisateur
      ...
      console.log("Token généré:", token);  // Log le token généré
  
      res.json({ token });
    } catch (error) {
      console.error("Erreur serveur:", error);  // Log l'erreur si elle se produit
...
app.put('/api/users/ficheuser/:id', authenticateToken, async (req, res) => {
  ...
  const { nom, prenom, adresse, ville, codepostal,datenaissance, mailcontact, telephone, role, idCNX, signature, idFicheMere, numSS, TransportDispo, Valide } = req.body;
...
app.post('/api/users/completetaxi', upload.fields([
  ...
  try {
    ...
    await ficheUtilisateur.update({
      ...
      datenaissance: etape2.datenaissance,
...
app.get('/inscriptuser/product/:id', async (req, res) => {
  try {
...
app.post('/inscriptuser/create-payment-intent', async (req, res) => {
